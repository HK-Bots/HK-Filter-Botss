...
async def start(client, message):
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            if not await check_verification(client, message.from_user.id) and not await db.has_premium_access(message.from_user.id) and VERIFY == True:
                btn = [[
    ...
    if not files_:
        ...
        try:
            if  not await check_verification(client, message.from_user.id) and not await db.has_premium_access(message.from_user.id) VERIFY == True:
                btn = [[
                    InlineKeyboardButton("✅ ᴠᴇʀɪꜰʏ ✅", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
                ]]
                await message.reply_text(
